...
def dividir_dados(dados: pd.DataFrame, coluna_alvo: str, classificador = False):
    ...
    # X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=0.1793, random_state=123456789, stratify=estratificar)

    print(f"Tamanhos das divisoes: Train={len(X_train)}, Test={len(X_test)}")
    # X_resampled, y_resampled = SMOTE(random_state=123456789).fit_resample(X_train, y_train)
...
def avaliar_modelos(X_teste, Y_teste, X_teste_normalizado, competencia=None):
    ...
    melhores_modelos = resultados_qwk
...
# arquivo_original_dataset = "grades_comparison_r1.csv"
arquivo_original_dataset = "grades_comparison_double_r1.csv"
...
if __name__ == "__main__":
    ...
    # Carrega os dados das outras execs
    dados_p_merge = []
